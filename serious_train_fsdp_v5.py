...
v5 fixes v4 OOM:
- root FSDP is retained for embedding/output sharding
- transformer blocks are grouped into 8 FSDP shards (4 blocks/group)
- no activation checkpointing
- explicit NCCL device_id
- persistent 65,536 BPE tokenizer
- real JSONL only; no synthetic/DPO/auto-pilot/resume
"""
...
def setup():
    ...
    return device
def make_cfg():
    v=VocabConfig(vocab_size=65536,byte_bpe_vocab=65536); c=NeuroCoreConfig(vocab=v)
    c.block.num_layers=32; c.block.alra.dim=1024; c.block.alra.num_heads=16; c.block.alra.head_dim=64; c.block.sgp.dim=1024
    c.moe.num_experts=1; c.moe.real_top1=False; c.bitnet.enabled=True; c.bitnet.quantize_mode='ternary'; c.bitnet.use_shadow_weights=True
    return c,v
def get_tokenizer(vcfg,dataset,model_dir):
    os.makedirs(model_dir,exist_ok=True); path=os.path.join(model_dir,'tokenizer_65536.json')
    if main_rank() and not os.path.isfile(path):
        corpus=os.path.join(model_dir,'tokenizer_corpus.txt'); log('Tokenizer not found -> building ONCE from all training records')
        extract_corpus_sample(dataset,corpus,max_lines=None); b=ByteBPETokenizer(vcfg); b.train([corpus],vocab_size=65536,special_tokens=list(vcfg.special_tokens.keys()))
        if b.vocab_size!=65536: raise RuntimeError(f'Tokenizer produced {b.vocab_size}; expected 65536')
        b.save(path); log(f'TOKENIZER CREATED: {path} | vocab=65,536')
    dist.barrier(); b=ByteBPETokenizer.load(path,vcfg)
...
def build_model(cfg,device,groups=8):
    model=NeuroCoreModel(cfg).to(device); mp=MixedPrecision(param_dtype=torch.float32,reduce_dtype=torch.float16,buffer_dtype=torch.float16)
    layers=list(model.layers)
    if len(layers)!=32: raise RuntimeError(f'Expected 32 layers, got {len(layers)}')
    grouped=nn.ModuleList()
    per=32//groups
    for i in range(groups):
        seq=nn.Sequential(*layers[i*per:(i+1)*per])
        grouped.append(FSDP(seq,sharding_strategy=ShardingStrategy.FULL_SHARD,mixed_precision=mp,device_id=device,use_orig_params=True))
...
def save_sharded(model,opt,step,root):
    ...
    if main_rank(): (d/'meta.txt').write_text(f'step={step}\nworld_size=2\nparams=675226178\n',encoding='utf-8'); (Path(root)/'LATEST').write_text(str(d),encoding='utf-8'); log(f'SHARDED CHECKPOINT SAVED: {d}')
    dist.barrier()
def main():
    p=argparse.ArgumentParser(); p.add_argument('--dataset',required=True); p.add_argument('--val-dataset',required=True); p.add_argument('--model-dir',required=True); p.add_argument('--steps',type=int,default=20); p.add_argument('--seq-len',type=int,default=512); p.add_argument('--batch-size',type=int,default=1); p.add_argument('--grad-accum',type=int,default=16); p.add_argument('--lr',type=float,default=1e-4); p.add_argument('--weight-decay',type=float,default=.1); p.add_argument('--warmup',type=int,default=2000); p.add_argument('--log-every',type=int,default=1); p.add_argument('--eval-every',type=int,default=1000); p.add_argument('--checkpoint-every',type=int,default=1000); p.add_argument('--data-workers',type=int,default=1); p.add_argument('--seed',type=int,default=42); a=p.parse_args()
    device=setup(); torch.manual_seed(a.seed+R()); random.seed(a.seed+R()); cfg,vcfg=make_cfg()
    if main_rank():
        log('============================================================'); log('TANTRA 0.675B — MEMORY-SAFE FAST 2-GPU FSDP v5'); log('============================================================'); log('675,226,178 params | vocab=65,536 | 32x1024 | 16 heads | dense | BitNet ternary'); log('8 FSDP groups x 4 blocks | NO activation checkpointing | REAL JSONL | NO synthetic/DPO/auto-pilot/resume/MTP/latent')
    ...
    opt=torch.optim.AdamW(model.parameters(),lr=a.lr,weight_decay=a.weight_decay,betas=(.9,.95),eps=1e-8); scaler=torch.amp.GradScaler('cuda',enabled=True); it=iter(train_loader); micro=0; step=0; local_tokens=0; start=time.time(); run_loss=0.; run_n=0; opt.zero_grad(set_to_none=True); model.train()
    while step<a.steps:
        try: x,y=next(it)
        except StopIteration: it=iter(train_loader); x,y=next(it)
        x,y=x.to(device,non_blocking=True),y.to(device,non_blocking=True); s=step+1; lr=a.lr*(s/max(1,a.warmup)) if s<=a.warmup else a.lr*.5*(1+math.cos(math.pi*min(1.,(s-a.warmup)/max(1,a.steps-a.warmup))))
        ...
        if micro%a.grad_accum==0:
            scaler.unscale_(opt); gn=torch.nn.utils.clip_grad_norm_(model.parameters(),.5); scaler.step(opt) if torch.isfinite(gn) else None; scaler.update(); opt.zero_grad(set_to_none=True); step+=1
            if step%a.log_every==0 or step==1:
                torch.cuda.synchronize(device); lt=torch.tensor(run_loss/max(1,run_n),device=device,dtype=torch.float64); dist.all_reduce(lt); lt/=WS(); speed=local_tokens*WS()/max(time.time()-start,1e-9)
