...
* **Safe Tensor Deserialization**: All checkpoint and weight loading pipelines use `weights_only=True` with `torch.serialization.add_safe_globals()` to allowlist known NeuroCore config dataclasses. This prevents arbitrary pickle code execution (RCE) while supporting custom checkpoint metadata.
