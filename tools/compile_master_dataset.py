...
Produces 5,000+ genuinely distinct, high-density samples across 8 core domains.
"""
import os
import re
import sys
...
def generate_multi_domain_curriculum() -> List[Dict[str, str]]:
    ...
    def add(u: str, a: str, dom: str = "general"):
        norm_u = u.strip()
        norm_a = a.strip()
    ...
    # =========================================================================
    # ── 1. Algorithms, Data Structures & Python (1,200+ samples) ──
    # =========================================================================
    algorithms = [
    ...
    for n in range(2, 60):
        add(f"Write a Python function to generate all prime numbers up to {n*10} using the Sieve of Eratosthenes.",
            f"```python\ndef sieve_of_eratosthenes(limit: int) -> list[int]:\n    is_prime = [True] * (limit + 1)\n    is_prime[0] = is_prime[1] = False\n    for p in range(2, int(limit**0.5) + 1):\n        if is_prime[p]:\n            for i in range(p * p, limit + 1, p):\n                is_prime[i] = False\n    return [p for p in range(2, limit + 1) if is_prime[p]]\n\nprimes = sieve_of_eratosthenes({n*10})\n```\n**Complexity:** $O(N \\log \\log N)$ time and $O(N)$ auxiliary space.",
            "code")

    # =========================================================================
    # ── 2. Mathematics, Calculus, Linear Algebra & Probability (1,800+ samples) ──
    # =========================================================================
    ...
    # Linear Equations (ax + b = c) with dynamic varied target x_ans
    for a in range(2, 25):
        for b in range(1, 25):
            x_ans = ((a * 5 + b * 11) % 23) + 1  # Distributed across x in [1, 23]
            c = a * x_ans + b
    ...
    for a11 in range(1, 8):
        for a12 in range(1, 8):
            for a21 in range(1, 8):
                for a22 in range(1, 8):
                    det = a11 * a22 - a12 * a21
                    add(f"Find the determinant of the 2x2 matrix [[{a11}, {a12}], [{a21}, {a22}]].",
    ...
    # =========================================================================
    # ── 3. Natural Sciences (Physics, Chemistry, Biology) (1,000+ samples) ──
    # =========================================================================
    physics_formulas = [
    ...
    # Kinematics calculations with varying initial velocity, acceleration, and time
    ...
    for i_val in range(1, 15):
        for r_val in range(2, 20):
            v_val = i_val * r_val
            p_val = v_val * i_val
            add(f"In a circuit with current I = {i_val} A and resistance R = {r_val} Ohms, calculate the voltage and power.",
                f"1. **Voltage:** $V = IR = {i_val} \\times {r_val} = {v_val}\\text{{ Volts}}$\n2. **Power:** $P = VI = {v_val} \\times {i_val} = {p_val}\\text{{ Watts}}$\n\n**Results:** $V = {v_val}\\text{{ V}}$, $P = {p_val}\\text{{ W}}$",
                "science")

    # =========================================================================
    # ── 4. Multilingual & Hindi (हिंदी) Technical (300+ samples) ──
    # =========================================================================
    hindi_tech = [
    ...
    # =========================================================================
    # ── 5. Tool Calling (`<tool_call>`) (300+ samples) ──
    # =========================================================================
    ...
    # =========================================================================
    # ── 6. Reasoning, Logic & Safety Alignment ──
    # =========================================================================
    logic_puzzles = [
