...
def main():
    ...
    for _name, _help in [
        ("emotion", "Generate curated emotional-intelligence QA dataset"),
        ("spatial", "Generate curated spatial/geometry/navigation QA dataset"),
        ("action", "Generate curated action-planning/robotics QA dataset"),
        ("factual", "Generate curated factual QA dataset"),
        ("general", "Generate curated general-purpose QA dataset"),
    ]:
...
# ═══════════════════════════════════════════════════════════
# CURATED QA CATEGORIES (emotion / spatial / action / factual / general)
# Folded in from the former tools/generate_training_data.py so there is a
# single synthetic-data builder. code/reasoning stay in their own subcommands.
# ═══════════════════════════════════════════════════════════

CURATED_SYSTEM = {
    "emotion": "You are Atulya. Be empathetic, warm, and emotionally intelligent.",
    "spatial": "You are Atulya. Reason precisely about space, geometry, and navigation.",
    "action": "You are Atulya. Plan and execute physical actions with precision.",
    "factual": "You are Atulya. Answer clearly and accurately.",
    "general": "You are Atulya. Answer clearly and accurately.",
}
...
_CURATED_COMMANDS = tuple(CURATED_DATA.keys())
...
def _curated_topic_from_question(user: str) -> str:
    topic = user.rstrip("?.!").strip()
    for prefix in (
        "What is ", "What are ", "How do I ", "How does ", "Explain ",
        "Write ", "Tell me about ", "Can you explain ",
    ):
...
def _curated_answer_variant(assistant: str, rng: random.Random, idx: int) -> str:
    text = assistant.strip()
    if idx % 3 == 1 and not text.startswith(("Short answer:", "Step by step:")):
        text = rng.choice(CURATED_OPENERS) + text[0].lower() + text[1:]
    if idx % 4 == 2:
        text = text.rstrip() + rng.choice(CURATED_CLOSERS)
    return text


def _curated_main(args) -> None:
    category = args.command
    system = CURATED_SYSTEM[category]
    data = CURATED_DATA[category]
    target_rows = max(100, args.rows)

    rng = random.Random(args.seed)
    records = [{"system": system, "user": u, "assistant": a, "category": category} for u, a in data]
    seen = {(r["user"], r["assistant"]) for r in records}
    idx = 0
    while len(records) < target_rows and idx <= target_rows * 20:
        user, assistant = data[idx % len(data)]
        prefix, suffix = CURATED_VARIATIONS[idx % len(CURATED_VARIATIONS)]
        topic = _curated_topic_from_question(user)
        new_user = f"{prefix}{topic.lower()}{suffix}"
        if idx >= len(data) * len(CURATED_VARIATIONS):
            new_user = f"{new_user} Example set {idx // len(CURATED_VARIATIONS) + 1}."
        new_assistant = _curated_answer_variant(assistant, rng, idx)
        key = (new_user, new_assistant)
        if key not in seen:
            records.append({"system": system, "user": new_user, "assistant": new_assistant, "category": category})
            seen.add(key)
        idx += 1

    rng.shuffle(records)
    out_dir = CHAT_OUT_DIR / category
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / (args.output or f"synthetic_{category}_curated.jsonl")
    with open(out_path, "w", encoding="utf-8", newline="\n") as f:
        for r in records:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")
    print(f"[{category}] Generated {len(records)} curated pairs -> {out_path}")
