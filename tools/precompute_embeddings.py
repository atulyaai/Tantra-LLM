...
Each saved file is a dict:
    {
        "vision_embeds": torch.Tensor [vision_dim],   # == settings.model_dim, matches vision_projector.in_features
        "audio_embeds":  torch.Tensor [audio_dim],    # == settings.model_dim, matches audio_projector.in_features
...
from npdna.sensory import VisionEncoder, AudioEncoder, TextTokenizer
...
def parse_args():
    ...
    p.add_argument("--vision-dim",  type=int, default=4096,  help="Vision encoder output dim (must equal settings.model_dim)")
    p.add_argument("--audio-dim",   type=int, default=4096,  help="Audio encoder output dim (must equal settings.model_dim)")
...
def _encode_vision(encoder: VisionEncoder, path: str, vision_dim: int) -> torch.Tensor | None:
    ...
    try:
        embed = encoder.encode(path)          # shape [1, vision_dim]
        embed = embed.squeeze(0).cpu()        # [vision_dim]
        if embed.size(0) != vision_dim:
            logger.error(f"Vision encoder returned dim {embed.size(0)}, expected {vision_dim} for {os.path.basename(path)}. Skipping.")
            return None
...
def _encode_audio(encoder: AudioEncoder, path: str, audio_dim: int) -> torch.Tensor | None:
    ...
    try:
        ...
        embed = encoder.encode(waveform)      # shape [1, audio_dim]
        embed = embed.squeeze(0).cpu()        # [audio_dim]
        if embed.size(0) != audio_dim:
            logger.error(f"Audio encoder returned dim {embed.size(0)}, expected {audio_dim} for {os.path.basename(path)}. Skipping.")
            return None
        if embed.abs().sum().item() == 0.0:
...
def main():
    ...
    # Initialize tokenizer and encoders lazily with correct target projector dimensions
    tokenizer = TextTokenizer(args.tokenizer)
    vis_encoder = VisionEncoder(embed_dim=args.vision_dim) if image_map else None
    aud_encoder = AudioEncoder(embed_dim=args.audio_dim)  if audio_map else None
