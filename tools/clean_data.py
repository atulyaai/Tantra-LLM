"""Unified data cleaning pipeline with configurable passes.

Pass 1 (default): Full first-pass pipeline from clean_and_combine.py
  - Parse JSONL, quality filter, PII redact, dedup, classify, balance

Pass 2 (--strict): Stricter second-pass from cleanup_low_quality.py
  - Quality filter (min 8 words, 40-6000 chars), echo filter,
    low-info opener filter, near-dedup at higher threshold

Both (--pass both): Run pass 1 then pass 2 sequentially.

Usage:
    python tools/clean_data.py --pass 1
    python tools/clean_data.py --pass 2 --in train_pack/mega_clean_500k.jsonl
    python tools/clean_data.py --pass both --max-rows 500000
"""
...
# ── Boilerplate / low-value patterns (extended from cleanup_low_quality) ──
BOILERPLATE = re.compile(
    ...
    r"disclaimer|powered\s+by|theme\s+by|job\s+seeker|job\s+alert|"
    r"donate\s+now|sign\s+up\s+for\s+our|download\s+the\s+app|"
    r"this\s+is\s+an\s+advertisement|sponsored)"
)

# ── Generic, content-free chat openers (from cleanup_low_quality) ──
LOW_INFO_OPENERS = re.compile(
    r"(?is)^(.(?:just been learning|same question|everything is under control|"
    r"looking forward to|happy to help|good luck|i'm glad|let me know if you need|"
    r"thanks for reaching|have a great day|you're welcome|no problem{1,3})[^a-z]{3,100}$|"
    r"^.{0,60}(<3|:\)|:p|:d|;)\.?$)"
)
...
# ── DedupFilter (more complete version from clean_and_combine.py) ──
...
# ══════════════════════════════════════════════════════════════════════
# PASS 1: First-pass pipeline (clean_and_combine.py)
# ══════════════════════════════════════════════════════════════════════

def pass1_parse_row(line: str) -> dict[str, str] | None:
    try:
        raw = json.loads(line)
    except Exception:
        return None
...
def pass1_quality_filter(row: dict[str, str], min_words: int = 4) -> bool:
...
def run_pass1(args: argparse.Namespace, rng: random.Random) -> list[dict]:
    out_dir = OUT_DIR
    ...
    print("=" * 60)
    print("PASS 1 — STAGE 1: PARSING & QUALITY FILTERING")
    print("=" * 60)
    ...
    for path in jsonl_files:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for line in f:
                row = pass1_parse_row(line)
                if not row:
                    parse_errors += 1
                    continue
                if not pass1_quality_filter(row):
                    quality_dropped += 1
                    continue
                all_rows.append(row)
    ...
    print("\n" + "=" * 60)
    print("PASS 1 — STAGE 2: PII REDACTION & NORMALIZATION")
    print("=" * 60)
    pii_found = 0
    for row in all_rows:
        before_u = row["user"]
    ...
    print("\n" + "=" * 60)
    print(f"PASS 1 — STAGE 3: DEDUPLICATION (threshold={args.dedup_strength})")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PASS 1 — STAGE 4: CLASSIFICATION")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"PASS 1 — STAGE 5: BALANCING (target: {args.max_rows:,})")
    print("=" * 60)
...
# ══════════════════════════════════════════════════════════════════════
# PASS 2: Second-pass stricter cleanup (cleanup_low_quality.py)
# ══════════════════════════════════════════════════════════════════════

def pass2_quality_filter(row: dict, min_words: int = 8) -> tuple[bool, str]:
    user = clean_space(row.get("user") or "")
    assistant = clean_space(row.get("assistant") or "")
    if not user or not assistant:
        return False, "empty"
    if len(assistant) < 40 or len(assistant) > 6000:
        return False, "length"
    words = re.findall(r"[A-Za-z]+", assistant)
    if len(words) < min_words:
    ...
    if LOW_INFO_OPENERS.search(assistant.strip()):
        return False, "low_info_opener"

    # Echo filter: assistant that mostly repeats the user question makes poor
    # training data. Drop if the user string is largely contained in the reply.
    u_tokens = set(re.findall(r"[a-z0-9]+", user.lower()))
    a_tokens = set(re.findall(r"[a-z0-9]+", assistant.lower()))
    if u_tokens and a_tokens:
        overlap = len(u_tokens & a_tokens) / len(u_tokens)
        if user and len(user) > 30 and overlap > 0.85:
            return False, "echo"
    return True, "ok"


def run_pass2(args: argparse.Namespace, input_rows: list[dict] | None = None) -> list[dict]:
    if args.report:
        return []
    ...
    if input_rows is not None:
        rows = input_rows
        raw = len(rows)
        src_label = f"in-memory ({raw:,} rows from pass 1)"
    else:
        src = Path(args.inp)
        rows = []
        raw = 0
    ...
    print("\n" + "=" * 60)
    print(f"PASS 2 — QUALITY FILTER (strict) | source: {src_label}")
    print("=" * 60)

    reason = Counter()
    kept = []
    for row in rows:
        ok, why = pass2_quality_filter(row, min_words=args.min_words)
        if not ok:
            reason[why] += 1
            continue
        kept.append(row)

    print(f"  Input rows:            {raw:,}")
    print(f"  Passed quality filter: {len(kept):,}")
    print(f"  Drops by reason: {dict(reason.most_common())}")

    dedup = DedupFilter(threshold=args.dedup_strength)
    final = []
    for row in kept:
        ...
        if dedup.is_dup(text):
            continue
        final.append(row)
    print(f"  After dedup @ {args.dedup_strength}: {len(final):,}")
    print(f"    exact dups: {dedup.exact:,} | near dups: {dedup.near:,}")

    if input_rows is None:
        out = src.with_name(src.stem.replace("_expanded", "_expanded_clean") + ".jsonl")
        ...
        manifest = {
            ...
            "raw_rows": raw,
            "dedup_threshold": args.dedup_strength,
            "drop_reasons": dict(reason.most_common()),
            "categories": dict(cats.most_common()),
        }
        m_path = out.with_suffix(".json").with_name(out.stem + "_manifest.json")
...
def main():
    parser = argparse.ArgumentParser(description="Unified data cleaning pipeline")
    parser.add_argument("--pass", dest="run_pass", default="1",
                        choices=["1", "2", "both"],
                        help="1=first-pass, 2=strict second-pass, both=sequential")
    parser.add_argument("--max-rows", type=int, default=1_000_000,
                        help="Target rows (pass 1)")
    parser.add_argument("--dedup-strength", type=float, default=0.88,
                        help="Near-dedup threshold (0-1)")
    parser.add_argument("--output-name", default="mega_clean",
                        help="Output name prefix (pass 1)")
    parser.add_argument("--source-file", action="append", default=[],
                        help="Specific JSONL source file to clean (pass 1). Can be passed multiple times.")
    parser.add_argument("--in", dest="inp",
                        default="Download/train_pack/train_pack_all_expanded_1040k.jsonl",
                        help="Input JSONL for pass 2")
    parser.add_argument("--min-words", type=int, default=8,
                        help="Min assistant words (pass 2)")
    ...
    if args.run_pass == "1":
        ...
        run_pass2(args)
    elif args.run_pass == "both":
        balanced = run_pass1(args, rng)
        if balanced:
            run_pass2(args, input_rows=balanced)
        else:
            print("Pass 1 produced no rows; skipping pass 2.")
