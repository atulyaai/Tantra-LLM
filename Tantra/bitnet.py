...
from Tantra.utils import pack_4_ternary_to_1_byte, unpack_1_byte_to_4_ternary
...
class TernaryQuantizer:
    ...
    def pack(self, W_q: Tensor) -> Tensor:
        ...
        # Shift order [6, 4, 2, 0] (MSB-first) matches pack_4_ternary_to_1_byte()
        # in utils.py, which pack_uint8() below delegates to. Keeping both
        # packers on the same bit order makes int32 and uint8 packings of the
        # same weights bit-identical (see test_uint8_int32_packing_equivalence).
        W_mapped_u8 = (W_flat + 1).to(torch.uint8).view(-1, 4)
        shifts_u8 = torch.tensor([6, 4, 2, 0], dtype=torch.uint8, device=W_q.device)
        packed_u8 = (W_mapped_u8[:, 0] << shifts_u8[0]) | (W_mapped_u8[:, 1] << shifts_u8[1]) | (W_mapped_u8[:, 2] << shifts_u8[2]) | (W_mapped_u8[:, 3])
    ...
    def unpack(self, W_packed: Tensor, original_shape: tuple) -> Tensor:
        ...
        shifts_u8 = torch.tensor([6, 4, 2, 0], dtype=torch.uint8, device=W_packed.device)
