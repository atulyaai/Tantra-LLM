...
def execute_tool_call(tool_name: str, arguments: Dict[str, Any], sandbox_enabled: Optional[bool] = None) -> str:
    ...
    permissions. When None, reads the TANTRA_ENABLE_SANDBOX environment
    variable (default: disabled).

    SECURITY: this used to default to enabled ("1") when the env var was
    unset. python_executor is not actually a sandbox -- it's an unrestricted
    subprocess running the full Python interpreter with this process's OS
    permissions -- and both tools are triggered automatically by whatever
    <tool_call> block appears in the MODEL's own generated text, which can be
    steered by prompt injection from untrusted input the model reads (a
    fetched document, a tool result, pasted text, etc.). The only current
    caller (webui/server.py) always passes an explicit sandbox_enabled value,
    so it was never exploitable through the shipped webui -- but a bare env-
    var default of "enabled" is a footgun for any future integration that
    forgets to pass the flag explicitly. Fixed to fail closed: code execution
    and file access now require an explicit opt-in.
    """
    if sandbox_enabled is None:
        sandbox_enabled = os.environ.get("TANTRA_ENABLE_SANDBOX", "0") == "1"
