...
class MoERouter(nn.Module):
    ...
    def forward(
        ...
        # Compute probabilities across all experts (differentiable)
    ...
    def load_balancing_loss(self, selected_experts: Tensor | None = None) -> Tensor:
        """
        Compute differentiable load balancing loss to prevent expert collapse.
        Uses router probabilities across all experts.
        """
        probs = getattr(self, "last_routing_probs", None)
        if probs is None or probs.numel() == 0:
            if selected_experts is not None and selected_experts.numel() == 0:
        ...
        return torch.mean((mean_probs - ideal_fraction) ** 2) * self.config.load_balance_coeff
