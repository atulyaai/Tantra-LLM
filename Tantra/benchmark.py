...
from Tantra.eval_suite import IndustryBenchmarkSuite, EvaluationEngine
...
def run_benchmarks(checkpoint_path: str = None, device: str = "auto"):
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        ckpt = safe_load_checkpoint(checkpoint_path, map_location=dev)
    ...
    # Inference Speed Benchmark (via EvaluationEngine)
    engine = EvaluationEngine(model, device=dev.type)
    speed_results = engine.benchmark_throughput(batch_size=1, seq_len=128, num_runs=10, vocab_size=cfg.vocab.vocab_size)
    print(f"\n[Speed] -> {speed_results['tokens_per_sec']:.2f} tokens/sec ({speed_results['ms_per_token']:.2f} ms/token)")
    speed_results_4 = engine.benchmark_throughput(batch_size=4, seq_len=128, num_runs=10, vocab_size=cfg.vocab.vocab_size)
    print(f"[Speed] -> Batch 4: {speed_results_4['tokens_per_sec']:.2f} tokens/sec ({speed_results_4['ms_per_token']:.2f} ms/token)")
