...
from Tantra.utils import pack_2bit_to_4x, unpack_2bit_from_4x, safe_load_checkpoint
...
class ResidualPredictor(nn.Module):
    ...
    def load(cls, path: str, config: CompressionConfig) -> 'ResidualPredictor':
        """
        SECURITY: this used to call torch.load(path, map_location='cpu') with
        no weights_only flag, defaulting to full (unsafe) pickle deserialization
        on PyTorch < 2.6 -- an RCE if `path` ever came from an untrusted source
        (e.g. a shared/downloaded .dna weight package, which is exactly what
        this compression module is for). Every other checkpoint loader in the
        codebase already routes through Tantra.utils.safe_load_checkpoint
        (weights_only=True + add_safe_globals); this one was the one exception.
        Fixed by routing through the same safe helper.
        """
        model = cls(config)
        model.load_state_dict(safe_load_checkpoint(path, map_location='cpu'))
        return model
...
class DNACodec:
    ...
    def _pack_to_dna(self, data: bytes) -> bytes:
        ...
        packed = pack_2bit_to_4x(arr)
        return packed.tobytes()

    def _unpack_from_dna(self, dna_bytes: bytes, original_len: int) -> bytes:
        arr = np.frombuffer(dna_bytes, dtype=np.uint8)
        out = unpack_2bit_from_4x(arr, original_len)
        return out.tobytes()
    ...
    def _serialize_weights_binary(self, weights_dict: Dict[str, torch.Tensor]) -> bytes:
        ...
        for key, tensor in weights_dict.items():
            if tensor.dtype == torch.bfloat16:
                raw_bytes = tensor.float().cpu().contiguous().numpy().tobytes()
            else:
