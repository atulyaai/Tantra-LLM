...
class TokenJuiceEngine:
    ...
    def enrich_batch(self, x: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if x.shape[0] == 1:
            # Do not overwrite the only real sample in batch; concatenate synthetic sample
            x = torch.cat([x, x_syn.unsqueeze(0)], dim=0)
            y = torch.cat([y, y_syn.unsqueeze(0)], dim=0)
        else:
            idx = random.randint(0, x.shape[0] - 1)
            x[idx : idx + 1] = x_syn.unsqueeze(0)
            y[idx : idx + 1] = y_syn.unsqueeze(0)
        return x, y
    ...
    def __iter__(self) -> Iterator[Tuple[torch.Tensor, torch.Tensor]]:
        ...
        def _process_line(raw_line: str) -> List[Tuple[torch.Tensor, torch.Tensor]]:
            ...
            if self.mask_non_assistant:
                ...
                target_flags = is_target if self.mask_non_assistant else [True] * len(clamped)
                token_buffer.extend(clamped)
                mask_buffer.extend(target_flags)
