...
from .architecture import CodecConfig
...
def configure_cpu_runtime(num_threads: int | None = None) -> dict[str, int | str]:
    """Configure one consistent CPU thread budget for local inference/training."""
    ...
    try:
        ...
        pass
    return {"threads": threads, "cpu_capability": capability}


def enable_torch_cpu_optimizations(num_threads: int | None = None) -> None:
    """Enable conservative PyTorch CPU optimizations."""
    configure_cpu_runtime(num_threads)


def optimize_with_ipex(core: Any, dtype: torch.dtype | None = None) -> Any:
    """Apply Intel Extension for PyTorch when it is explicitly installed."""
    if not hasattr(core, "model"):
        raise TypeError("optimize_with_ipex expects an object with a model attribute")
    try:
        import intel_extension_for_pytorch as ipex
    except ImportError as exc:
        raise RuntimeError(
            "Intel Extension for PyTorch is not installed. Install a version compatible with your PyTorch build."
        ) from exc
    core.model.cpu().eval()
    core.model = ipex.optimize(core.model, dtype=dtype, inplace=True)
    return core


def prepare_cpu_inference(core: Any, num_threads: int | None = None) -> Any:
    """Put a loaded core into a deterministic, CPU-only inference posture."""
    if not hasattr(core, "model"):
        raise TypeError("prepare_cpu_inference expects an object with a model attribute")
    enable_torch_cpu_optimizations(num_threads)
    core.model.cpu().eval()
...
def benchmark_checkpoint(
    checkpoint: str | Path = "model/latest",
    *,
    max_tokens: int = 40,
    include_cpu_metrics: bool = False,
) -> dict:
    ...
    if include_cpu_metrics:
        result["cpu"] = benchmark_cpu_inference(
            core,
            [task["q"] for tasks in BENCHMARK_TASKS.values() for task in tasks],
            max_tokens=min(16, max_tokens),
        )
    return result


def benchmark_cpu_inference(core: NpDnaCore, prompts: list[str], *, max_tokens: int = 16, context_window: int = 256) -> dict:
    """Measure first-token latency and decode throughput for CPU inference."""
    if not prompts:
        raise ValueError("prompts must not be empty")
    prepare_cpu_inference(core)
    samples, first_times = [], []
    total_chars = total_decode_seconds = 0.0
    for prompt in prompts:
        started = time.perf_counter()
        stream = core.generate_stream(prompt, max_tokens=max_tokens, temperature=0.0, top_k=1, context_window=context_window)
        try:
            first = next(stream)
            first_seconds = time.perf_counter() - started
            text = first + "".join(stream)
        except StopIteration:
            first_seconds, text = time.perf_counter() - started, ""
        elapsed = time.perf_counter() - started
        first_times.append(first_seconds)
        total_chars += len(text)
        total_decode_seconds += max(0.0, elapsed - first_seconds)
        samples.append({"prompt": prompt, "first_token_seconds": first_seconds, "total_seconds": elapsed, "chars": len(text)})
    ordered = sorted(first_times)
    p95_index = min(len(ordered) - 1, max(0, math.ceil(len(ordered) * 0.95) - 1))
    return {
        "samples": samples,
        "prompt_count": len(samples),
        "first_token_seconds_p50": ordered[len(ordered) // 2],
        "first_token_seconds_p95": ordered[p95_index],
        "decode_chars_per_second": total_chars / max(total_decode_seconds, 1e-9),
        "threads": torch.get_num_threads(),
    }


def compile_readiness() -> dict[str, str | bool]:
    """Report whether this host can use TorchInductor's Windows CPU backend."""
    compiler = next((name for name in ("cl", "clang", "icx") if shutil.which(name)), None)
    return {
        ...
        "ready": bool(compiler),
        "next_step": "benchmark eager versus torch.compile" if compiler else "install/activate MSVC, LLVM, or Intel Compiler",
    }
...
def write_benchmark(
    ...
    *,
    ...
    result = benchmark_checkpoint(checkpoint, max_tokens=max_tokens, include_cpu_metrics=include_cpu_metrics)
...
def benchmark_main() -> None:
    ...
    parser.add_argument("--checkpoint", default="model/latest")
    parser.add_argument("--output", default=None)
    parser.add_argument("--max-tokens", type=int, default=40)
    parser.add_argument("--cpu", action="store_true", help="Also measure first-token latency and decode throughput.")
    args = parser.parse_args()
    result = write_benchmark(args.checkpoint, args.output, max_tokens=args.max_tokens, include_cpu_metrics=args.cpu)
    ...
    if "cpu" in result:
        print(f"first_token_p50={result['cpu']['first_token_seconds_p50']:.3f}s")
        print(f"first_token_p95={result['cpu']['first_token_seconds_p95']:.3f}s")
