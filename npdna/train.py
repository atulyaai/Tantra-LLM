...
def refresh_dataset_sizes(data_dir: Path) -> None:
    """Refresh curriculum size estimates from local JSONL files."""
    for folder in all_folders:
        folder_path = data_dir / folder
        if not folder_path.exists():
            DATASET_SIZES[folder] = 0
            continue
        total_bytes = sum(
            fp.stat().st_size
        ...
        DATASET_SIZES[folder] = max(1, int(total_bytes / (1024 * 1024))) if total_bytes else 0
...
def save_training_state(path: Path, opt=None, scaler=None) -> None:
...
def load_training_state(path: Path, opt, scaler=None, device: torch.device | str = "cpu") -> bool:
    ...
    try:
        ...
        print(f"  Optimizer state skipped: {str(exc)[:120]}")
        return False
    return True
...
def save_training_checkpoint(
    ...
    save_training_state(ckpt_path, opt=opt, scaler=scaler)
...
def train(
    ...
    print(f"  NP-DNA: {CONFIG_NAME}, attn={use_attention}, device={device_obj}")
    ...
    if freeze_vocab:
        ...
        if len(dataset.seed_chat_records):
    ...
    if compile_model and multi_gpu:
        print("  torch.compile skipped: --multi-gpu uses DataParallel")
    ...
    if start_step > 1 and not reset_step_on_resume:
        if load_training_state(resume_dir, opt, scaler=scaler, device=device_obj):
