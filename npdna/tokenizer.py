...
class AtulyaTokenizer:
    ...
    def dynamic_vocab_growth(
        ...
        """Grow vocabulary from a bounded sample, automatically finding all
        unique subword pairs in the data.

        When target_vocab_size=0, BPE runs until it naturally exhausts all
        frequent pairs (min_pair_freq threshold), so the vocabulary grows to
        exactly what the data requires — no manual target needed.
        ...
        if target_vocab_size and target_vocab_size <= self.size:
            if return_stats:
                return 0, {
        ...
        if target_vocab_size:
            # Grow toward the requested target, honoring the caller's cap on
            # BPE merge rounds; the forced-chunk fallback fills any shortfall.
            remaining_vocab = max(0, target_vocab_size - self.size)
            effective_merge_rounds = min(merge_rounds, remaining_vocab)
        else:
            # Automatic: run BPE until data is exhausted (no more frequent pairs)
            # Use a very large limit; train_bpe stops naturally when heap empties
            effective_merge_rounds = 1_000_000

        target_merges = len(self.merges) + effective_merge_rounds
        if self.max_capacity is not None:
            # Never exceed model embedding capacity
