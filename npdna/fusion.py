"""Multimodal dataset for fusion training.

Fusion projectors are the model's built-in ``nn.Linear(4096, H)`` layers in
``npdna/model.py``, trained jointly with the text LM via ``npdna/train.py
--fusion`` and saved into the normal checkpoint. This module only provides the
dataset used by that path.

Single flat module (previously ``training/`` + ``training/datasets/``).
"""
