...
def get_model_and_tokenizer(checkpoint_path: Optional[str] = None):
    ...
    if MODEL is None or TOKENIZER is None or checkpoint_path is not None:
        ...
        if os.path.exists(ckpt_path):
            try:
                # Use safe_load_checkpoint which registers NeuroCore config dataclasses
                # as safe globals and loads with weights_only=True (no RCE risk).
                from Tantra.utils import safe_load_checkpoint
                ckpt = safe_load_checkpoint(ckpt_path, map_location=device)
                ...
                MODEL.eval()
                # BitLinear registers its packed-weight buffers as None until
                # to_inference_mode() runs, so load_state_dict silently drops
                # 'packed_weight'/'weight_scale' keys from a quantized
                # checkpoint (they don't exist as real tensors on a fresh
                # module yet) -- previously this meant a quantized export
                # loaded with weight=None and crashed on first forward().
                # Materialize those buffers first for any layer the
                # checkpoint has packed weights for.
                from Tantra.bitnet import BitLinear
                for _name, _module in MODEL.named_modules():
                    if isinstance(_module, BitLinear) and f"{_name}.packed_weight" in state:
                        if not _module.is_inference:
                            _module.to_inference_mode()
