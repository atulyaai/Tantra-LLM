"""Fast 2x-T4 FSDP trainer for Tantra 0.675B.

v4 performance path:
- root-only FSDP FULL_SHARD (avoids 32 nested FSDP collectives)
...
def setup():
    ...
    if n < 2:
        raise RuntimeError(f"Worker sees only {n} CUDA device(s). CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')!r}")
    local = LR()
    if local < 0 or local >= n:
        raise RuntimeError(f"LOCAL_RANK={local} but worker sees {n} GPUs")
...
def build_model(cfg, device):
    # Root-only FSDP: one shard manager instead of 32 nested FSDP modules.
    # This reduces NCCL all-gather/reduce-scatter launch overhead substantially.
...
def evaluate(model, loader, device, max_batches):
    ...
    ns = torch.zeros((), device=device, dtype=torch.float64)
    with torch.no_grad():
...
def main():
    ...
    p.add_argument("--lr", type=float, default=1e-4); p.add_argument("--weight-decay", type=float, default=.1); p.add_argument("--warmup", type=int, default=2000); p.add_argument("--log-every", type=int, default=5); p.add_argument("--eval-every", type=int, default=100); p.add_argument("--checkpoint-every", type=int, default=100); p.add_argument("--data-workers", type=int, default=1); p.add_argument("--seed", type=int, default=42)
    a = p.parse_args(); device = setup(); torch.manual_seed(a.seed + R()); random.seed(a.seed + R())
    if main_rank():
        log("============================================================"); log("TANTRA 0.675B — FAST REAL 2-GPU FSDP TRAINING v4"); log("============================================================"); log("675,226,178 params | vocab=65,536 | 32x1024 | 16 heads | dense | BitNet ternary"); log("FSDP FULL_SHARD root-only | NO activation checkpointing | REAL JSONL | NO synthetic | NO resume | NO DPO | NO auto-pilot | NO MTP | NO latent reasoning"); log(f"seq={a.seq_len} | batch/GPU={a.batch_size} | grad_accum={a.grad_accum} | GLOBAL TOKENS/OPT STEP={a.seq_len*a.batch_size*a.grad_accum*WS():,}")
    ...
    while step < a.steps:
        ...
        if micro % a.grad_accum == 0:
            ...
            if step % a.log_every == 0 or step == 1:
                torch.cuda.synchronize(device); lt = torch.tensor(run_loss/max(1, run_n), device=device, dtype=torch.float64); dist.all_reduce(lt); lt /= WS(); elapsed = time.time() - start; speed = local_tokens * WS() / max(elapsed, 1e-9)
