...
def test_multimodal_vision_encoding():
    ...
    global_visual_ids = (visual_token_ids + vcfg.image_range_start).clamp(0, cfg.vocab.total_embedding_size - 1)
    with torch.no_grad():
        logits, _ = model.forward(global_visual_ids)

    assert logits.shape == (1, visual_token_ids.shape[1], cfg.vocab.total_embedding_size)
...
def test_omnimodal_assembly():
    ...
    audio_ids = (audio_tokens + vcfg.audio_range_start).clamp(0, vcfg.total_embedding_size - 1)
    image_ids = (image_tokens + vcfg.image_range_start).clamp(0, vcfg.total_embedding_size - 1)
    ...
    assert logits.shape == (1, omnimodal_input.shape[1], cfg.vocab.total_embedding_size)
...
def test_python_executor(monkeypatch):
    monkeypatch.setenv('TANTRA_ENABLE_SANDBOX', '1')
    code = "print(sum(range(10)))"
    res = execute_tool_call("python_executor", {"code": code})
    assert res == "45"

def test_file_reader(monkeypatch):
    monkeypatch.setenv('TANTRA_ENABLE_SANDBOX', '1')
