...
def test_real_gradient_flow_non_zero():
    ...
    loss = nn.functional.cross_entropy(logits.view(-1, logits.shape[-1]), y.view(-1))
    loss.backward()
...
def test_real_bpe_tokenizer_trained_vocab():
    ...
    # Core Hindi words that must be efficiently encoded
    test_words = ["आप", "उत्तर", "गणित", "भारत", "नहीं", "यह", "है"]
    ...
    # At least 5 of these 7 core domain words must be efficiently tokenized
    assert single_token_count >= 5, (
        f"Tokenizer vocabulary quality is poor: only {single_token_count}/7 domain words are efficiently tokenized"
    )
