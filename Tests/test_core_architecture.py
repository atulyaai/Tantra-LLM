...
def test_model_forward():
    ...
    assert logits.shape == (1, 16, cfg.vocab.total_embedding_size)
...
def test_latent_cot_header():
    ...
    assert logits_with_cot.shape == (1, 8, cfg.vocab.total_embedding_size)
    assert logits_no_cot.shape == (1, 8, cfg.vocab.total_embedding_size)
...
def test_real_top1_moe_dispatches_and_exposes_balance_loss():
    ...
    assert logits.shape == (2, 8, cfg.vocab.total_embedding_size)
...
def test_cpu_causal_attention_profile_forward():
    ...
    assert logits.shape == (1, 8, cfg.vocab.total_embedding_size)
...
def micro_config():
    ...
    # Shrink the modality codebooks too (not just vocab_size) so this stays a
    # genuinely "micro" embedding/output table (~1064 rows) instead of the
    # 1000 + 3*8192 = 25576 rows a default-codebook VocabConfig would give.
    cfg.vocab = VocabConfig(
        vocab_size=1000,
        audio_codebook_size=16, image_codebook_size=32, video_codebook_size=16,
    )
...
def test_mtp_dual_token_forward():
    ...
    assert logits_main.shape == (1, seq_len, cfg.vocab.total_embedding_size)
    assert logits_mtp.shape == (1, seq_len, cfg.vocab.total_embedding_size)

    # Restrict to the text-vocab slice: the output layer covers the full
    # shared multimodal range, so an untrained model's unrestricted argmax
    # can legitimately land in the (unused here) audio/image/video range.
    pred_t1 = torch.argmax(logits_main[:, -1, :cfg.vocab.vocab_size], dim=-1).item()
    pred_t2 = torch.argmax(logits_mtp[:, -1, :cfg.vocab.vocab_size], dim=-1).item()
